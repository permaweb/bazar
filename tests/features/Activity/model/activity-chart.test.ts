...
import { chartHoverIndex, globalActivityChartStats } from 'features/Activity/model/activity-chart';
...
describe('global activity chart statistics', () => {
	...
	it('normalizes millisecond timestamps into the same buckets as second timestamps', () => {
		const seconds = globalActivityChartStats([event('a', 'transfer', 'wallet-a', 1_700_000_000)]);
		const milliseconds = globalActivityChartStats([event('a', 'transfer', 'wallet-a', 1_700_000_000_000)]);
	...
	it('maps pointer positions to the nearest bounded chart bucket', () => {
		...
		expect(chartHoverIndex(100, 100, 300, 0)).toBeNull();
		expect(chartHoverIndex(Number.NaN, 100, 300, 3)).toBeNull();
	});
});
