...
export default function TokenMarketRow(props: {
	...
	return (
		...
		>
			<S.Logo className="token-market-logo">
				<TokenAvatar
			...
			<S.Identity className="token-market-identity">
				<strong>{props.asset.name}</strong>
				<small>{ticker}</small>
			</S.Identity>
			<S.Context className="token-market-context">{props.context ?? props.asset.id}</S.Context>
			...
			{props.badge ? <S.Badge className="token-market-badge">{props.badge}</S.Badge> : null}
...
function TokenMetric(props: { metric: TokenMarketMetric; className?: string }) {
	return (
		<S.Metric className={`token-market-metric ${props.className ?? ''} ${props.metric.tone ?? 'default'}`}>
