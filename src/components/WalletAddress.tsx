		...
		<>
			...
			>
				...
				{copyState === 'copied'
					...
					: copyState === 'failed'
						? <><small>Copy failed</small><CircleAlert className="ui-icon ui-icon--xs" aria-hidden="true" /></>
						: <Copy className="ui-icon ui-icon--xs" aria-hidden="true" />}
			</button>
			<span className="sr-only" aria-live="polite" role="status">
				{copyState === 'copied' ? `${label} address copied.` : copyState === 'failed' ? `Could not copy ${label} address.` : ''}
			</span>
		</>
	);
}
...
export function WalletIdentity({ address }: { address: string }) {
	return <span className="wallet-identity" title={address}>{address}</span>;
}
