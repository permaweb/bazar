...
const PROGRESS_STOPS = [
	{ at: 0, color: new THREE.Color('#f4d985') },
	{ at: 0.12, color: new THREE.Color('#f2b500') },
	{ at: 0.45, color: new THREE.Color('#c9ea2d') },
	{ at: 0.72, color: new THREE.Color('#69d36d') },
							...
							<AcceptedProofPayloads>
								{proof.recalls.slice(0, 2).map((recall) => (
									<AcceptedProofPayload
										...
										href={
											recall.content?.kind === 'binary' ? undefined : recall.content?.contentUrl
										}
...
function progressColor(progress: number, target = new THREE.Color()): THREE.Color {
	...
	const stopIndex = Math.max(0, PROGRESS_STOPS.findIndex((stop, index) => index > 0 && normalized <= stop.at) - 1);
	const start = PROGRESS_STOPS[stopIndex];
	const end = PROGRESS_STOPS[Math.min(PROGRESS_STOPS.length - 1, stopIndex + 1)];
	const mix = start === end ? 0 : (normalized - start.at) / (end.at - start.at);
	return target.copy(start.color).lerp(end.color, Math.min(1, Math.max(0, mix)));
}
