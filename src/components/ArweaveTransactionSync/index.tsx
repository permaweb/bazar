...
export function ArweaveTransactionSync({ subject, steps, activeStep, onProgressChange }: Props) {
	...
	const acceptedProofs = React.useMemo(
		() =>
			miningTelemetry.acceptedProofs.filter(
				(proof) =>
					proof.recallSamples.length > 0 &&
					proof.recallSamples.every((sample) => sample.content !== undefined)
			),
		[miningTelemetry.acceptedProofs]
	);
	const cableLanes = React.useMemo(
		() => lanes.map((lane) => infinity3DLane(lane, observedSteps, liveAt, false, language)),
