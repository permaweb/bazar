...
/**
 * The creation page shell: the `.create-*` and `.mint-*` classes the asset creator, the media picker and the
 * fungible token fields share. The holder dispatch feature renders the same class names and carries its own copy
 * in `features/Dispatch/styles/create-form.ts`: the two are separate lazy routes and a feature may not import
 * another feature's internals, so the declarations are stated once per feature instead of shared.
 */
...
/** Hidden native picker opened from a visible control. */
