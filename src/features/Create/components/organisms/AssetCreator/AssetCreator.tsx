...
import { waitForAssetState } from 'api/marketplace';
import {
	AssetMintClient,
...
function UdlGrantField(props: {
	...
	const needsValue = props.value && ['revenue-share', 'one-time', 'monthly'].includes(props.value.grant);
	return (
		<div className={needsValue ? 'udl-field udl-grant-field has-value' : 'udl-field udl-grant-field'}>
			...
			<div className={needsValue ? 'udl-field-control with-value' : 'udl-field-control'}>
				<Select
					...
					onChange={(grant) => {
						if (!grant) return props.onChange(undefined);
						props.onChange({
...
export default function AssetCreator() {
	...
	const customUdlLicenseIdValid = isArweaveId(normalizedCustomUdlLicenseId);
						...
						<div className="mint-artwork-field">
							...
							<div>
								<Button type="button" onClick={() => artworkInput.current?.click()} size="custom">
									<Icon icon={Upload} size="sm" /> {artwork ? 'Replace artwork' : 'Add artwork'}
								</Button>
								{artwork ? (
									<Button
										type="button"
				...
				>
					...
					{result || collectionResult ? (
						...
						>
							...
							{!working ? <Icon icon={ArrowRight} /> : null}
						</Button>
					)}
				</form>
			</div>
