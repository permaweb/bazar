...
import { type Observer, type ObserverView, type PurchaseTransaction } from 'api/transactions';
...
import {
	...
	timelineLayout,
	transitionKey,
} from '../../../model/raceTimeline';
import { confirmationLifecycleState, sequencePhaseBounds } from '../../../model/sequence';
...
export default function ArweaveTransactionSync(props: Props) {
	...
	const verificationDelayed = observerVerificationDelayed(active);
	...
	const observedSteps = useLiveObserverResponses(props.steps);
	const telemetrySessionKey = transactionSyncSessionKey(observedSteps);
	const telemetrySessionRef = React.useRef({ key: telemetrySessionKey, startedAt: props.startedAt ?? Date.now() });
	if (
		telemetrySessionRef.current.key !== telemetrySessionKey ||
		(props.startedAt !== undefined && telemetrySessionRef.current.startedAt !== props.startedAt)
	) {
		telemetrySessionRef.current = { key: telemetrySessionKey, startedAt: props.startedAt ?? Date.now() };
	}
	...
	const liveAt = useRaceClock((props.active ?? true) && raceActive);
	...
	const miningTelemetry = useArweaveMiningTelemetry(
		(props.miningTelemetryEnabled ?? true) && (props.active ?? true) && lanes.length > 0,
		telemetrySessionKey,
		telemetrySessionRef.current.startedAt
	);
	...
	React.useEffect(() => {
		...
		props.onProgressChange?.(activePhaseProgress);
	}, [activePhaseProgress, confirmedProgress, props.onProgressChange, progressKey]);
		...
		<>
						...
						<S.Depth
							aria-label={
								terminalDepthBeyondTarget
									...
									: `${displayedConfirmationDepth} of ${target}`
							}
			...
			{lanes.length > 0 && (
				<TransactionVisualizerBoundary
					...
					resetKey={transaction?.id ?? props.subject}
				>
					<>
