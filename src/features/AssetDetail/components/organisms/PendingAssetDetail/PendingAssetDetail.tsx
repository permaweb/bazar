...
export default function PendingAssetDetail() {
	...
	return (
		<S.Page className="mint-pending-page">
			<Link className="back" to="/">
				<Icon icon={ArrowLeft} size="sm" /> {messages.pendingContinueBrowsing}
			</Link>
			<S.Layout className="mint-pending-layout">
				<S.Artwork className="mint-pending-artwork">
					{mint.asset.image ? (
						<ArtworkImage
						...
						<S.ArtworkFallback className="mint-pending-artwork-fallback" aria-hidden="true">
							{mint.asset.name.slice(0, 1)}
						</S.ArtworkFallback>
					)}
				</S.Artwork>
				<S.Copy className="mint-pending-copy">
					<Eyebrow>{messages.pendingEyebrow}</Eyebrow>
					<h1>{mint.asset.name}</h1>
					<p>{messages[PHASE_STATUS_KEYS[mint.activity.phase]] as string}</p>
					<S.Phases className="mint-pending-phases">
					...
					<S.GatewayNote className="mint-pending-gateway">
						{view.pinnedGateway ? messages.pendingGatewayNotePinned : messages.pendingGatewayNote}
					</S.GatewayNote>
					<S.Actions className="mint-pending-actions">
