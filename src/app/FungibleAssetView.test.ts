...
describe('fungible operation error semantics', () => {
	it('counts only the new wallet approvals missing from a recovered batch', () => {
		expect(batchPurchaseRecoveryApprovalCount([
			{ snapshot: { registration: { id: REGISTRATION_ID, dispatched: true }, payment: { id: PAYMENT_ID, dispatched: false } } },
	...
	it('keeps a 512-order completion bounded while every exact receipt remains selectable', () => {
		...
		const purchaseStates = Object.fromEntries(orders.map((order, index) => [order.orderId, {
		...
		const receipt = renderToStaticMarkup(React.createElement(FungiblePurchaseReceiptNavigator, {
		...
		const firstReceipt = renderToStaticMarkup(React.createElement(FungiblePurchaseReceiptNavigator, {
	...
	it('makes every matched seller reachable through one bounded keyboard region', () => {
		...
		const review = renderToStaticMarkup(React.createElement(MatchedListingsReview, {
			matchMode: 'amount',
			orders,
			state: { denomination: 0, ticker: 'WEAVE' } as AssetState,
		}));
	...
	it('keeps interactive settlement recovery outside the assertive alert summary', () => {
		const alert = renderToStaticMarkup(React.createElement(FungibleOperationErrorAlert, {
...
describe('fungible batch payment coordination', () => {
	...
	it('resumes only batch orders still available to the same buyer', () => {
		...
		expect(fungibleBatchRecoveryStatus(
			resume,
			state({ [ORDER_ID]: openOrder }),
			BUYER,
		)).toBe('resumable');
		expect(fungibleBatchRecoveryStatus(
			resume,
	...
	it('distinguishes recovery attempts for the same order by their signed transactions', () => {
		...
		expect(batchRecoveryIdentity([
			{ order, snapshot: { registration: { id: 'registration-a', dispatched: false } } },
		])).not.toBe(batchRecoveryIdentity([
			{ order, snapshot: { registration: { id: 'registration-b', dispatched: false } } },
		]));
	});
	...
	it('waits for every sibling before reporting a batch failure', async () => {
		...
		let reported = false;
		const result = waitForSettlementBatch([
			Promise.reject(new Error('reservation failed')),
			sibling,
		]).catch((cause) => {
			reported = true;
			throw cause;
	...
	it('releases a mixed resumed batch after only its remaining reservation becomes ready', () => {
		expect(batchPaymentBarrierState([
			{
				snapshot: {
	...
	it('does not let an idle purchase event erase its prepared registration', () => {
		...
		expect(latestRecoverableSnapshot(prepared, {
	...
	it('keeps complete recovery evidence through partial resumed purchase states', () => {
		...
		expect(latestRecoverableSnapshot(complete, {
	...
	it('persists a fully signed batch before honoring a late abort', () => {
		...
		expect(() => storeBatchRecoveryBeforeDispatch(
			storage,
...
describe('manual listing keyboard navigation', () => {
	...
	it('keeps selected listings first without scanning membership quadratically', () => {
		const reads = { count: 0 };
		const orders = Array.from({ length: 10_000 }, (_, index) => new Proxy(
			{ orderId: `order-${index}` },
			{ get(target, property, receiver) { if (property === 'orderId') reads.count += 1; return Reflect.get(target, property, receiver); } },
		));
...
describe('fungible order action names', () => {
	it('distinguishes each whole-lot action by quantity, total, and seller', () => {
		...
		expect(fungibleOrderActionLabel('cancel', first, state)).toBe(
	...
	it('distinguishes sellers whose compact identities collide', () => {
		...
		expect(fungibleListingAccessibleLabel(first, state)).not.toBe(
			fungibleListingAccessibleLabel(second, state),
		);
	});
});
...
describe('fungible transfer recipient validation', () => {
	...
	it('names the exact recipient before an irreversible transfer', () => {
		...
		expect(fungibleTransferSubmitLabel('2000000000000', state, recipient)).toBe(
