...
describe('Home market summary retries', () => {
	...
	it('announces large activity scans only at bounded batch milestones', () => {
		const messages = new Set(Array.from({ length: 160 }, (_, index) =>
			collectionActivityScanAnnouncement({
				error: false,
				events: index * 3,
				loading: true,
		...
		expect(collectionActivityScanAnnouncement({
			error: false,
			events: 18,
			loading: false,
			pages: 160,
	...
	it('selects unavailable and still-pending summaries for an in-place retry', () => {
		...
		expect(retryableHomeSummaryKeys(
			['listed', 'empty', 'throttled', 'index', 'pending'],
			summaries,
		)).toEqual(['throttled', 'index', 'pending']);
	});

	it('starts only new work while an unchanged summary remains in flight', () => {
		expect(homeSummaryRequestKeys(
			['existing', 'arrived'],
			{},
			['existing'],
			new Set(),
...
describe('Home collection activity windows', () => {
	...
	it('deduplicates recipients and resets after collection members are removed', () => {
		...
		commitHomeActivityBatch(scan, [
			{ processId: kept, height: 20 } as any,
			{ processId: removed, height: 10 } as any,
		], [kept, removed]);
	...
	it('rejects foreign candidates before committing a discovery batch', () => {
		...
		expect(() => commitHomeActivityBatch(
			scan,
			[{ processId: foreign, height: 10 } as any],
			[requested],
		)).toThrow('home-activity-batch-out-of-scope');
...
describe('Home collection floor retries', () => {
	...
	it('retains 999 live contributions and retries only one failed candidate', () => {
		...
		let scan = reconcileHomeFloorScan(undefined, 'scope-a', candidates.map((processId) => activity(processId)));
		...
		scan = reconcileHomeFloorScan(scan, 'scope-a', candidates.map((processId) => activity(processId)));
	...
	it('treats a verified no-ask result as settled and prunes removed minima', () => {
		...
		const other = 'other';
		let scan = reconcileHomeFloorScan(undefined, 'scope-a', [noAsk, minimum, other].map((id) => activity(id)));
		...
		scan = reconcileHomeFloorScan(scan, 'scope-a', [noAsk, other].map((id) => activity(id)));
		expect(pendingHomeFloorCandidates(scan)).toEqual([]);
...
describe('Collection activity scope', () => {
	...
	it('changes with a names namespace replacement', () => {
		...
		expect(collectionActivityVersion(collection)).not.toBe(collectionActivityVersion({
			...collection,
	...
	it('keeps listing scope stable when a paged token window only grows', () => {
		...
		expect(collectionListingScopeVersion(collection)).toBe(collectionListingScopeVersion({
			...collection,
			assets: [...collection.assets, { id: 'B'.repeat(43), name: 'Another asset' }],
	}));
	});
...
describe('Collection listing announcements', () => {
	...
	it('holds failure churn until a milestone and reports exact completion immediately', () => {
		...
		const belowMilestone = nextListingAnnouncementProgress(initial, {
			scope: 'collection', resolved: 7, failures: 7, total: 100, loading: true,
		});
		expect(belowMilestone).toEqual(initial);
		expect(nextListingAnnouncementProgress(belowMilestone, {
			scope: 'collection', resolved: 7, failures: 7, total: 100, loading: false,
...
describe('Collection activity windows', () => {
	it('deduplicates completion-order results and retains the exact newest limit', () => {
		...
		expect(newestCollectionActivity([
			...events.slice(100),
			...events.slice(0, 100),
			events[204],
		]).map((event) => event.id)).toEqual(
			events.slice(105).reverse().map((event) => event.id),
		);
	});
});
...
describe('Collection live listing truth', () => {
	...
	it('merges a resolution batch once with last-outcome truth', () => {
		...
		expect(mergeResolvedListingBatch([first, second, untouched], [
