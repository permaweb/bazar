...
describe('marketplaceErrorMessage', () => {
	it.each([
		['asset-purchase-insufficient-funds-after-signing', 'Add AR, then resume the same signed transaction.'],
		['transaction-propagation-timeout', 'The signed transaction is saved and safe to resume.'],
		['asset-state-timeout', 'Resume to keep checking live state.'],
		['asset-order-reservation-expired', 'No seller payment was sent'],
		['wallet-account-changed', 'Reconnect the original signer to resume safely.'],
		...
		['fungible-transfer-proof-mismatch', 'signed transaction is saved'],
		['asset-cancel-rejected', 'listing changed first'],
		['asset-cancel-proof-mismatch', 'signed transaction is saved'],
		['asset-purchase-rejected', 'permanent payment evidence'],
		['asset-purchase-proof-mismatch', 'Both exact transaction IDs remain saved'],
		['asset-payment-id-missing', 'cannot prove settlement safely'],
		['asset-action-starting-slot-unavailable', 'did not ask the wallet'],
		['asset-action-recovery-baseline-missing', 'cannot safely infer its outcome'],
	...
	it('retains structured lifecycle rejection codes through user-facing copy', () => {
		...
		expect(marketplaceErrorMessage(failure)).toContain('accepted reservation remains safe');
	});
});
