const FRIENDLY_ERRORS: Record<string, string> = {
	...
	'asset-order-reservation-expired':
		'This reservation no longer has enough time to reach scheduled state safely. No seller payment was sent; the exact signed transactions remain stored for review.',
	...
	'market-state-changed':
		'The owner or listing changed since it was last checked. Close this dialog and review the newly verified market state before approving anything.',
	'registration not found':
		'Arweave nodes accepted the reservation, but it was not mined during this observation window. Choose the visible Resume action to keep watching the exact signed transaction without signing again.',
	'payment not found':
		'Arweave nodes accepted the payment, but it was not mined during this observation window. Choose the visible Resume action to keep watching the exact signed payment without paying again.',
	...
	'fungible-transfer-proof-mismatch':
		'The selected compute gateway returned incomplete scheduler proof for this transfer. The signed transaction is saved; resume it here, or close this dialog and choose another Compute gateway in the header.',
	...
	'asset-cancel-proof-mismatch':
		'The selected compute gateway returned incomplete scheduler proof for this cancellation. The signed transaction is saved; resume it here, or close this dialog and choose another Compute gateway in the header.',
	...
	'asset-purchase-proof-mismatch':
		'The selected compute gateway returned incomplete scheduler proof for this purchase. Both exact transaction IDs remain saved; resume here, or close this dialog and choose another Compute gateway in the header.',
	...
	'asset-action-recovery-baseline-missing':
		'This older signed action has no exact process-slot baseline, so Bazar cannot safely infer its outcome from aggregate state. The signed transaction remains stored for review.',
	'transaction-dispatch-rejected':
		'Arweave rejected this exact signed transaction. Nothing was submitted; discard it before asking your wallet to sign a corrected replacement.',
	'registration-dispatch-rejected':
		'Arweave rejected the signed reservation. No seller payment was sent. Review the current listing before signing a replacement.',
	'payment-dispatch-rejected':
		'Arweave rejected the signed seller payment. The accepted reservation remains safe; resume it to sign only a replacement payment.',
};
...
export function marketplaceOperationFailure(error: unknown): MarketplaceOperationFailure {
	...
	return ['fungible-transfer-rejected', 'asset-cancel-rejected', 'asset-purchase-rejected'].includes(value) || [
		'transaction-dispatch-rejected',
		'registration-dispatch-rejected',
		'payment-dispatch-rejected',
	].includes(code) ? 'transaction-rejected' : 'other';
}
