...
export function hasRecoverablePurchase(snapshot?: {
...
export function removeCompletedPurchaseRecoveryAndSignatures<T>(
	...
	return status === 'complete' && removeWalletRecoveryAndSignatures(
		storage,
		key,
		matches,
		ids,
...
export function assertWalletOperationAvailable(
...
export async function acquireWalletOperationClaim(
	...
	void locks.request(claimKey, { mode: 'exclusive', ifAvailable: true }, async (lock) => {
		if (!lock) {
...
export function releaseWalletOperationClaim(
	...
	removeWalletRecordIf<{ attemptId?: string }>(
		storage,
		claim.key,
		(record) => record.attemptId === claim.attemptId,
	);
	claim.releaseLock();
}
