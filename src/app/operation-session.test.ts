...
describe('wallet-bound operation sessions', () => {
	...
	it('gives each signer independent recovery keys', () => {
		...
		expect(atomicPurchaseStorageKey('asset', 'wallet-a')).not.toBe(
			atomicPurchaseStorageKey('asset', 'wallet-b'),
		);
	});
	...
	it('requires an explicit action before recovery can request any new wallet approval', () => {
		...
		expect(purchaseRecoveryApprovalCount({
			registration: { id: REGISTRATION_ID, dispatched: true },
			payment: { id: PAYMENT_ID, dispatched: false },
		})).toBe(0);
		...
		expect(shouldAutomaticallyResumePurchase({
			registration: { id: REGISTRATION_ID, dispatched: true },
			payment: { id: PAYMENT_ID, dispatched: false },
		})).toBe(true);
	});
	...
	it('repairs only the terminally rejected purchase leg', () => {
		...
		expect(repairRejectedPurchase({
			registration: { id: REGISTRATION_ID, dispatched: true },
			payment: { id: PAYMENT_ID, dispatched: false },
		}, 'payment-dispatch-rejected')).toEqual({
	...
	it('retains signed material when a newer recovery record owns the storage key', () => {
		...
		expect(removeWalletRecoveryAndSignatures<{ attemptId: string }>(
			storage,
			key,
	...
	it('retains exact signed payment evidence while a purchase is blocked', () => {
		...
		expect(removeCompletedPurchaseRecoveryAndSignatures<{ buyer: string }>(
			'blocked',
			storage,
			recoveryKey,
	...
	it('removes an invalid record only from the signer-scoped key', () => {
		...
		expect(loadWalletRecord<{ signer: string }>(
			storage,
			scoped,
			legacy,
			(record) => record.signer === 'wallet-b',
	...
	it('removes only the exact attempt that finished', () => {
		...
		expect(removeWalletRecordIf<{ txId: string }>(storage, key, (record) => record.txId === 'older-transaction')).toBe(false);
		expect(values.has(key)).toBe(true);
		expect(removeWalletRecordIf<{ txId: string }>(storage, key, (record) => record.txId === 'newer-transaction')).toBe(true);
		expect(values.has(key)).toBe(false);
	});
	...
	it('does not let an older attempt overwrite newer recovery', () => {
		...
		expect(
			storeWalletRecordIf(storage, key, { txId: 'older-transaction', stage: 2 }, (record) => record.txId === 'older-transaction'),
		).toBe(false);
		expect(JSON.parse(values.get(key)!)).toEqual({ txId: 'newer-transaction', stage: 1 });
		expect(
			storeWalletRecordIf(storage, key, { txId: 'newer-transaction', stage: 2 }, (record) => record.txId === 'newer-transaction'),
		).toBe(true);
	...
	it('turns a lost recovery claim into a hard pre-dispatch failure', () => {
		...
		expect(() => storeWalletRecordOrThrow(
			storage,
			key,
	...
	it('holds one cross-document claim across deferred wallet approval', async () => {
		...
		await expect(acquireWalletOperationClaim(storage, claimKey, [operationKey, purchaseKey], {
			locks,
			recovery: {
				key: operationKey,
	...
	it('fails closed when cross-document locking is unavailable', async () => {
		...
		const claimKey = operationClaimStorageKey('asset', 'wallet-a');
		await expect(acquireWalletOperationClaim(storage, claimKey, [], { locks: null }))
			.rejects.toThrow('wallet-operation-lock-unavailable');
		expect(values.has(claimKey)).toBe(false);
	});
		...
		const queuedLocks = {
			request: <T,>(_name: string, _options: unknown, callback: (lock: unknown | null) => T | PromiseLike<T>) =>
				new Promise<T>((resolve, reject) => {
					grantLock = () => { void Promise.resolve(callback({})).then(resolve, reject); };
				}),
		};
	...
	it('distinguishes claim release from deliberate recovery removal', () => {
		...
		expect(walletOperationStorageChange(operationStorageKey('asset', 'wallet-b'), null, claimKey, recoveryKeys)).toBe('ignore');
	});
});
