...
describe('My assets retry bookkeeping', () => {
	...
	it('lets candidate discovery continue while live-state pages resolve in order', async () => {
		...
		const thirdWasOffered = new Promise<void>((resolve) => { reportThirdOffered = resolve; });
	...
	it('stops later live-state pages and reports the first resolver failure', async () => {
		...
		const queue = walletPageResolutionQueue(async ([page]) => {
			started.push(page.height);
			if (page.height === 1) await new Promise<void>((_, reject) => { rejectFirst = reject; });
		}, new AbortController().signal);
	...
	it('does not focus a completion summary after another asset arrives', () => {
		...
		expect(assetGroupRevealAnnouncement(96, 97, 'owned assets')).toBe(
			'Showing 96 of 97 owned assets.',
		);
		expect(assetGroupRevealAnnouncement(97, 97, 'owned assets')).toBe(
	...
	it('announces large candidate discovery at bounded milestones', () => {
		...
		const complete = nextWalletAnnouncementProgress(milestone, {
			...status,
			phase: 'done',
	...
	it('announces terminal ownership revalidation only at bounded milestones', () => {
		...
		for (let revalidated = 0; revalidated <= 104; revalidated += 1) {
			...
			announcements.add(walletResolutionCopy({
				...status,
	...
	it('keeps interrupted candidate discovery indeterminate', () => {
		...
		expect(walletResolutionIsDeterminate({
			...interrupted,
			phase: 'done',
			discoveryComplete: true,
			error: null,
		})).toBe(true);
		expect(walletResolutionShowsProgress({
			...interrupted,
			phase: 'discovering',
			error: null,
		})).toBe(true);
