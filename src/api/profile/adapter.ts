...
export function profileAvatarUrl(profile: AccountProfile | null | undefined, gateway = arweaveGatewayFromLocation()) {
	...
	return isArweaveId(id) ? arweaveDataUrl(id, gateway) : undefined;
}
...
function normalizeAvatar(value: string): string {
	if (isArweaveId(value)) return `ar://${value}`;
	if (value.startsWith('ar://') && isArweaveId(value.slice(5))) return value;
	try {
