...
export function hasRecoverablePurchase(
	...
	if (!registration || !isArweaveId(registration.id ?? '')) return false;
	if (!snapshot?.payment) return true;
	return isArweaveId(snapshot.payment.id ?? '');
}

export function purchaseRecoveryApprovalCount(snapshot?: PurchaseSnapshot | null) {
	if (isArweaveId(snapshot?.payment?.id ?? '')) return 0;
	if (isArweaveId(snapshot?.registration?.id ?? '')) return 1;
	return 2;
}
...
export function discardNewlyPreparedTransactionIfAborted(
	...
	if (newlyPrepared && isArweaveId(id)) {
		storage.removeItem(`bazar-signed-transaction:${id}`);
	}
	return true;
}
		...
		try {
			...
			const ids = [
				...
				...(Array.isArray(record?.entries)
					...
					: []),
			].filter((id): id is string => typeof id === 'string' && isArweaveId(id));
