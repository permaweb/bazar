...
export function liquidBalanceOf(state: AssetState, address: string): string {
	return isArweaveId(address) ? state.balances[address] ?? '0' : '0';
}
...
export function listedBalanceOf(state: AssetState, address: string): string {
	if (!isArweaveId(address)) return '0';
...
async function linkedRecord(
	...
	const entries = await Promise.all(
		Object.entries(message).flatMap(([name, value]) => {
			if (isArweaveId(name)) return [Promise.resolve<[string, unknown]>([name, value])];
			if (!name.endsWith('+link') || typeof value !== 'string') return [];
			const key = name.slice(0, -5);
			return isArweaveId(key) && isArweaveId(value)
				? [
						read(value).then(async (child): Promise<[string, unknown]> => {
							...
							const decoded =
								typeof child['order-id'] === 'string' && (!status || /^\d{3}$/.test(String(status)))
									...
									: child;
							const exactKey =
								typeof decoded['order-id'] === 'string' && isArweaveId(decoded['order-id'])
									? decoded['order-id']
									: key;
...
async function flattenLinkedTrie(
	...
	const visit = async (
		...
		if (nodeValue !== undefined && isArweaveId(prefix)) balances[prefix] = nodeValue;
		await Promise.all(
			Object.entries(node).flatMap(([rawName, value]) => {
				...
				if (!linked) {
					if (isArweaveId(owner)) balances[owner] = value;
					return [];
				}
