  ...
  it('signs a proportional partial fill against the original order', async () => {
    ...
    expect(registration.tags).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: Buffer.from('order-id').toString('base64url'), value: Buffer.from(order.orderId).toString('base64url') }),
        expect.objectContaining({ name: Buffer.from('fill-quantity').toString('base64url'), value: Buffer.from('1000000000000').toString('base64url') }),
      ]),
    );
