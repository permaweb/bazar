...
function client(
	...
	return { storage, arweave, requests, client: new AssetTransactionClient({
		wallet: {
...
describe('fungible asset transactions', () => {
	...
	it('verifies listing acceptance from uncached live process state', async () => {
		const subject = client(undefined, async () => Response.json({
			'execution-device': 'token@1.0',
			'total-supply': '10',
			balances: { [seller]: '9' },
	...
	it('rejects a noncanonical raw wire tag before persistence', async () => {
		...
		await expect(subject.client.transfer(processId, recipient, '12500000000000', seller))
	...
	it.each([
		['native quantity', (transaction: any) => { transaction.quantity = '1'; }],
		['process target', (transaction: any) => { transaction.target = recipient; }],
		['recipient', (transaction: any) => {
			transaction.tags.find((tag: { name: string }) => tag.name === 'recipient').value = seller;
		}],
		['token quantity', (transaction: any) => {
			transaction.tags.find((tag: { name: string }) => tag.name === 'quantity').value = '1';
		}],
		['action', (transaction: any) => {
			transaction.tags.find((tag: { name: string }) => tag.name === 'action').value = 'make-offer';
		}],
		['reward', (transaction: any) => { transaction.reward = '999999'; }],
	...
	it('rechecks approved intent immediately before dispatch', async () => {
		...
		await expect(prepared.dispatch(new AbortController().signal)).rejects.toBeInstanceOf(TransactionDispatchNotSentError);
		expect(subject.requests).toEqual([]);
	});

	it('classifies a demonstrated invalid-transaction response as terminal', async () => {
		const subject = client(undefined, async (input) => String(input).endsWith('/tx')
	...
	it('rejects incomplete exact payment evidence before reading it as settlement', () => {
		...
		const mutations: Array<(raw: any) => void> = [
			(raw) => { raw.process = recipient; },
			(raw) => { raw.body.commitments[transactionId].committer = seller; },
			(raw) => { raw.body.target = recipient; },
			(raw) => { raw.body['order-id'] = recipient; },
			(raw) => { raw.body.quantity = '26'; },
			(raw) => { raw.body.commitments[transactionId].committed = ['quantity', 'target']; },
			(raw) => { delete raw.body.commitments[transactionId]['field-target']; },
		];

		for (const mutate of mutations) {
			const raw = structuredClone(validRaw);
			mutate(raw);
			expect(() => assertExactPurchaseAssignment(
	...
	it('verifies exact historical settlement after the buyer spends the purchased units', async () => {
		...
		await expect(adapter.verifyOwnership!({
	...
	it('rejects incomplete exact cancellation evidence before reading it as success', () => {
		...
		const mutations: Array<(raw: any) => void> = [
			(raw) => { raw.process = recipient; },
			(raw) => { raw.body.commitments[transactionId].committer = recipient; },
			(raw) => { raw.body.action = 'transfer'; },
			(raw) => { raw.body['order-id'] = recipient; },
			(raw) => { raw.body.target = recipient; },
			(raw) => { raw.body.commitments[transactionId].committed = ['action', 'target']; },
			(raw) => { delete raw.body.commitments[transactionId]['field-target']; },
		];
		...
		for (const mutate of mutations) {
			const raw = structuredClone(validRaw);
			mutate(raw);
			expect(() => assertExactCancelAssignment(
				{ slot: 11, blockHeight: 51, transactionIds: [transactionId], raw },
				processId,
				transactionId,
				seller,
	...
	it('waits for the exact cancellation and treats a missing exact transition as rejection', async () => {
		...
		await expect(subject.waitForExactCancellation(
			processId,
			transactionId,
			seller,
			expected,
	...
	it('proves an exact-slot transfer from its paired notices rather than noisy balances', () => {
		...
		expect(fungibleTransferAppliedAtSlot(
			rejected,
			assignment,
			processId,
			transactionId,
	...
	it('distinguishes incomplete scheduler proof from an exact token rejection', () => {
		...
		const mutations: Array<(raw: any) => void> = [
			(raw) => { raw.process = recipient; },
			(raw) => { raw.body.commitments[transactionId].committer = recipient; },
			(raw) => { raw.body.recipient = seller; },
			(raw) => { raw.body.quantity = '11'; },
			(raw) => { raw.body.target = recipient; },
			(raw) => { raw.body.commitments[transactionId].committed = ['action', 'quantity', 'target']; },
			(raw) => { delete raw.body.commitments[transactionId]['field-target']; },
		];
		...
		for (const mutate of mutations) {
			const raw = structuredClone(validRaw);
			mutate(raw);
			expect(() => assertExactFungibleTransferAssignment(
				{ slot: 11, blockHeight: 51, transactionIds: [transactionId], raw },
				processId,
				transactionId,
				seller,
	...
	it('reports incomplete exact-assignment evidence without reading it as rejection', async () => {
		...
		await expect(subject.waitForFungibleTransfer(
			processId,
			transactionId,
			seller,
			recipient,
	...
	it('rediscovers schedule boundaries after a same-height reorg moves the exact transaction', async () => {
		...
		try {
			...
			const subject = new AssetTransactionClient({
				fetch: async (input: RequestInfo | URL) => {
					...
					if (schedule) {
						...
						const assignments: Record<number, unknown> = {};
						for (let slot = Number(schedule[1]); slot <= Number(schedule[2]); slot += 1) {
							const blockHeight = moved
								? slot < 10 ? 50 : slot === 10 ? 51 : 52
								: slot < 11 ? 50 : 51;
							assignments[slot] = scheduledAssignment(
								slot,
			...
			const pending = subject.waitForFungibleTransfer(
				processId,
				transactionId,
				seller,
				recipient,
	...
	it('does not start queued price requests after quote cancellation', async () => {
		...
		const atLimit = new Promise<void>((resolve) => { reachedLimit = resolve; });
		const subject = new AssetTransactionClient({
	...
	it('passes quote cancellation through every price request', async () => {
		...
		await expect(subject.estimatePurchaseBatchCosts([
			swapOrder(transactionId, '3000000000000', '1000000'),
		], processId, controller.signal)).rejects.toBe(reason);
	...
	it.each([3, 4])('discards every earlier batch approval when wallet prompt %i is rejected', async (rejectAt) => {
		...
		await expect(subject.client.preparePurchaseBatch(orders.map((order) => ({
			processId,
			order,
			buyer: seller,
			startingBalance: '0',
	...
	it('discards a pre-signed batch when navigation aborts its final balance check', async () => {
		...
		const controller = new AbortController();
		const preparation = subject.client.preparePurchaseBatch(orders.map((order) => ({
			processId,
			order,
			buyer: seller,
			startingBalance: '0',
	...
	it('refuses to dispatch a stored registration after its exact order disappears', async () => {
		...
		await expect(resumed.restorePrepared!(
			'registration',
			prepared.id,
	...
	it('stops an expired reservation on its first fresh-state check', async () => {
		...
		await expect(adapter.waitForRegistrationAcceptance!({
			registrationId: 'R'.repeat(43),
			signal: new AbortController().signal,
			report: () => undefined,
...
function approvalSubject(
	...
	const balanceStarted = new Promise<void>((resolve) => { announceBalanceStarted = resolve; });
	...
	const state = {
		...
		orders: Object.fromEntries(orders.map((order) => [order.orderId, {
			'order-id': order.orderId,
			creator: order.creator,
			recipient: order.recipient,
			asking: order.asking,
