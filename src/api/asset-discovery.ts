...
export function createWalletCandidateScan(
	address: string,
	graphql = PAGINATED_GRAPHQL,
): WalletCandidateScan {
...
export async function discoverWalletAssetCandidates(
	...
	const requestPage = async (
		...
		const { response, body: payload } = await fetchJsonWithDeadline<any>(fetcher, graphql, {
			method: 'POST',
		...
		return {
			requestedAliases,
			connections: new Map(requestedAliases.map((alias) => [
				alias,
...
export async function discoverMarketActivityBatched(
	options: BatchedMarketActivityOptions,
): Promise<AssetCandidate[]> {
	const recipients = [...new Set(options.recipients.filter((id) => ADDRESS.test(id)))];
	...
	const batches = Array.from(
		{ length: Math.ceil(recipients.length / batchSize) },
		(_, index) => recipients.slice(index * batchSize, (index + 1) * batchSize),
	);
	const found = new Map<string, AssetCandidate>();
	let nextBatch = 0;
...
function sortCollectionActivity(events: CollectionActivityEvent[]) {
	return events.sort((a, b) =>
		b.height - a.height || b.timestamp - a.timestamp || a.id.localeCompare(b.id));
}
...
async function verifyProcessDevices(
	...
	while (true) {
		...
		cursor = advanceGraphqlCursor(
			connection,
			visited,
			'collection-activity-device-pagination-stalled',
		);
	}
}
...
function decodeGraphqlConnection(payload: any, key: string, errorCode: string): GraphqlConnection {
	...
	if (
		...
		connection.edges.some((edge: any) =>
			!edge ||
...
function restrictAssetCandidatesWithIndex(
	candidates: AssetCandidate[],
	index: CandidateCollectionIndex,
) {
...
export async function verifyAssetCandidateSupport(
	...
	const batchSize = graphqlHost === 'arweave.net' || graphqlHost.endsWith('.arweave.net')
	...
	const workers = Array.from(
		{ length: Math.min(ASSET_SUPPORT_CONCURRENCY, chunks.length) },
		async () => {
	...
	return {
		supported: restrictAssetCandidates(candidates, collections).filter((candidate) =>
			supportedIds.has(candidate.processId) || verifiedIds.has(candidate.processId)
		),
...
export function partitionAssetCandidateSupport(
	...
	for (const candidate of restrictAssetCandidatesWithIndex(candidates, index)) {
		if (
			index.byAssetId.has(candidate.processId) ||
			candidateMatchesFungibleContract(candidate)
		) {
...
function candidateMatchesFungibleContract(candidate: AssetCandidate): boolean {
	return candidate.processDevice === 'process@1.0' &&
...
function activityEventFromNode(node: GraphqlNode): CollectionActivityEvent | null {
	...
	return {
		...
		height: safeNumber(node.block?.height),
		timestamp: safeNumber(node.block?.timestamp),
		...(action === 'make-offer' && /^[1-9]\d*$/.test(tags.asking ?? '')
			? { asking: tags.asking }
			: {}),
		...(['make-offer', 'transfer'].includes(action) && /^[1-9]\d*$/.test(
			action === 'make-offer' ? tags['offer-quantity'] ?? '' : tags.quantity ?? '',
		)
		...
		...(['register-interest', 'cancel-order'].includes(action) && ADDRESS.test(tags['order-id'] ?? '')
			...
			: {}),
		...(action === 'transfer' && ADDRESS.test(tags.recipient ?? '')
