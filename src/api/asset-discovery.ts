...
export async function discoverWalletAssetCandidates(
  ...
  while (active.size) {
    ...
    await options.onPage?.(sortCandidates([...new Map(pageCandidates.map((item) => [item.processId, item])).values()]));
  }
...
export async function discoverCollectionActivity(
  ...
  while (events.length < limit) {
    ...
    if (events.length >= limit || !connection?.pageInfo?.hasNextPage || !edges.length) {
      return events;
    }
    const nextCursor = edges.at(-1)?.cursor;
...
export function restrictAssetCandidates(candidates: AssetCandidate[], collections: Collection[]): AssetCandidate[] {
  ...
  return candidates.filter((candidate) => {
    ...
    if (candidate.bazarMint) return true;
...
function supportedAsset(
  ...
  const imageCollection = collections.find(
    (collection) => collection.kind === 'images' && collection.assets.some((asset) => asset.id === activity.processId),
  );
  if (imageCollection) {
  ...
  if (activity.bazarMint && computed.state.device === 'token@1.0') {
    const asset = assetFromMintState(activity.processId, computed.state.raw, computed.state.name);
    if (!asset) return null;
    const collection = createdCollection([asset]);
    return { asset, collection, state: computed.state, provider: computed.provider, activity };
  }
  ...
  return {
    ...
    ...(source === 'initial-holder'
      ? {
          ...
          bazarMint: isBazarMintTags(tags),
        }
      : {}),
  };
}
...
function mergeCandidate(found: Map<string, AssetCandidate>, next: AssetCandidate): void {
  ...
  found.set(next.processId, {
    ...
    bazarMint: current.bazarMint ?? next.bazarMint,
  });
}
