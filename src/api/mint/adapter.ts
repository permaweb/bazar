...
import { acceptedMintActivity, mintActivityId, removeMintActivities, upsertMintActivity } from './activity';
import {
...
import { AtomicAssetUploader, normalizeUploadTags } from './uploader';
...
export function getMintDraft(
	...
	if (!isArweaveId(owner) || !storage) return null;
	try {
...
export function discardMintDraft(
	...
	if (isArweaveId(owner)) storage?.removeItem(`${DRAFT_PREFIX}${owner}`);
}
...
function validateMintDraft(value: unknown): asserts value is MintDraft {
	...
	if (
		...
		!isArweaveId(draft.mediaId) ||
		!draft.name?.trim() ||
		draft.name.length > 80 ||
		!isSupportedAssetContentType(draft.contentType) ||
		(draft.artworkId !== undefined && !isArweaveId(draft.artworkId)) ||
