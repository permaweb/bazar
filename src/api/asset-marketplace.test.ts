...
describe('asset state', () => {
	...
	it('bypasses cached process state throughout transaction acceptance polling', async () => {
		const processId = 'IyFfmbTu8P4rv0KyrA0Q-QtfEnYntMj4RkRiBVip9KA';
		const requested: string[] = [];
		const result = await waitForAssetState(
			processId,
			(state) => state.balances[owner] === '1',
			{
				fetch: async (input) => {
					requested.push(String(input));
					if (requested.length === 1) return new Response('unsupported codec', { status: 415 });
	...
	it('reads one exact historical state and rejects a mismatched slot', async () => {
		...
		await expect(readAssetStateAtSlot(processId, 18, { fetch: fetcher as typeof fetch }))
			.resolves.toMatchObject({ state: { raw: { 'at-slot': 18 } }, maxAge: 0 });
		expect(requested[0]).toContain('compute?slot=18&require-codec=json%401.0');
		await expect(readAssetStateAtSlot(processId, 17, { fetch: fetcher as typeof fetch }))
			.rejects.toThrow('historical-state-slot-mismatch');
	});
	...
	it('rejects an incomplete schedule window instead of skipping a transaction', async () => {
		await expect(readProcessAssignments('R'.repeat(43), 8, 9, {
	...
	it('does not amplify a rate limit across codec fallbacks', async () => {
		...
		await expect(readAssetState('R'.repeat(43), { fetch: fetcher as typeof fetch }))
