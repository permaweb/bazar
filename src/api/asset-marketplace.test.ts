...
describe('asset state', () => {
	...
	it('projects binary data out of process state and resolves linked tables independently', async () => {
		...
		expect(requests.map(({ url }) => url)).toContain(`https://compute.example/${balancesLink}`);
		expect(requests.map(({ url }) => url)).toContain(`https://compute.example/${ordersLink}`);
		expect(
			requests.find(({ url }) => url === `https://compute.example/${balancesLink}`)?.headers.get('accept')
		).toBe('application/json');
		expect(requests.every(({ url }) => !url.includes(`${balancesLink}~message@1.0`))).toBe(true);
