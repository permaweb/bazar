				...
				start() {
					queueMicrotask(() => {
						if (control.outcome === 'timeout') control.listeners.get('timeout')?.();
							...
							const consensus = {
								state: 'confirmed', confirmations: 5, answering: 3, eligible: 3,
								agreeing: 3, quorum: 2, best: 5, seen: 3, propagated: true,
								blockId: 'block', blockHeight: 12, settled: true,
							};
...
describe('transaction dispatch observation', () => {
	...
	it('terminates a hung dispatch when transaction observation times out', async () => {
		const transaction = {
			...
			dispatch: (signal: AbortSignal) => new Promise<void>((_resolve, reject) => {
	...
	it('reports the exact watcher consensus used to settle the transaction', async () => {
		...
		expect(consensuses[0]).toMatchObject({
			state: 'confirmed', confirmations: 5, blockId: 'block', blockHeight: 12, settled: true,
		});
