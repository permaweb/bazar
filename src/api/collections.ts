...
export async function loadCollections(signal?: AbortSignal): Promise<Collection[]> {
  ...
  return [names.value, ...images.flatMap((result) => (result.status === 'fulfilled' ? [result.value] : []))];
}
...
export async function loadCollectionReference(referenceId: string, signal?: AbortSignal): Promise<Collection> {
  if (!/^[A-Za-z0-9_-]{43}$/.test(referenceId)) throw new TypeError('invalid-collection-reference');
  return loadImageCollection(referenceId, '', signal);
}
