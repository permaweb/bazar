	...
	return [
		...source.map((collection) => {
			...
			if (collection.kind === 'tokens' && replacement.kind === 'tokens') {
				...
				if (
					retained.indexSource !== 'compiled-fallback' &&
					replacement.indexSource === 'compiled-fallback'
				) return retained;
				const refreshed = new Map(replacement.assets.map((asset) => [asset.id, asset]));
				const sameWindow = replacement.assets.every(
					(asset, index) => retained.assets[index]?.id === asset.id,
				);
				const currentIsAhead = sameWindow && (retained.cursorHistory?.length ?? 0) >
					(replacement.cursorHistory?.length ?? 0);
			...
			return {
				...
				assets: [
					...collection.assets,
					...replacement.assets.filter((asset) => !seen.has(asset.id)),
				],
...
export async function loadMoreFungibleTokens(
	collection: Collection,
	signal?: AbortSignal,
): Promise<Collection> {
...
async function loadFungibleTokenPage(after?: string, signal?: AbortSignal): Promise<FungibleTokenPage> {
	const result = await fetchJsonWithDeadline<any>(fetch, `${DEFAULT_GATEWAY}/graphql`, {
		method: 'POST',
	...
	if (
		...
		connection.edges.some((edge: any) =>
			...
			edge.node.tags.some((tag: any) =>
				!tag || typeof tag.name !== 'string' || typeof tag.value !== 'string'
			)
		)
...
async function loadNames(signal?: AbortSignal): Promise<Collection> {
	const [namespace, page] = await Promise.all([
		loadNamesNamespace(signal),
		loadCarrierPage(undefined, signal),
	]);
...
async function loadCarrierPage(after?: string, signal?: AbortSignal): Promise<CarrierPage> {
	const { response, body: payload } = await fetchJsonWithDeadline<any>(fetch, PAGINATED_GRAPHQL, {
		method: 'POST',
		headers: { 'content-type': 'application/json' },
		body: JSON.stringify({
...
async function fetchJson<T>(path: string, signal?: AbortSignal, process = false): Promise<T> {
	if (!process && /^[A-Za-z0-9_-]{43}$/.test(path)) {
		const { response, body: responseBody } = await fetchTextWithDeadline(fetch, `${DEFAULT_GATEWAY}/tx/${path}/data`, { signal }, {
	...
	const { response, body } = await fetchJsonWithDeadline<any>(fetch, url, { signal, headers: { accept: 'application/json' } }, {
