...
import { HIDDEN_ASSET_IDS, HIDDEN_COLLECTION_IDS } from './policy';

export { HIDDEN_ASSET_IDS, HIDDEN_COLLECTION_IDS } from './policy';
...
export function collectionAsset(collection: Collection, id: string, state?: AssetState): AssetSummary | undefined {
	if (!isArweaveId(id) || !isVisibleAssetId(id)) return undefined;
...
export function fungibleAssetFromState(id: string, state?: AssetState): AssetSummary | undefined {
	...
	return {
		...
		...(typeof logo === 'string' && isArweaveId(logo)
...
export function replaceHiddenCollectionAssetIndex(index: Readonly<Record<string, readonly string[]>>): void {
	...
	for (const collectionId of HIDDEN_COLLECTION_IDS) {
		...
		if (assetIds) HIDDEN_COLLECTION_ASSET_IDS.set(collectionId, new Set(assetIds.filter((id) => isArweaveId(id))));
	}
}
...
const IMAGE_COLLECTIONS = [
	...
	(collection): collection is { reference: string; manifest: string } =>
		Boolean(collection.reference && isArweaveId(collection.reference)) &&
...
function indexedAtomicAsset(node: AtomicAssetIndexNode): AssetSummary | undefined {
	...
	if (
		...
		tags.denomination !== '0' ||
		tags.ticker !== 'ASSET' ||
		!isArweaveId(tags['initial-holder'] ?? '')
	)
		return undefined;
...
async function loadFungibleTokenPage(after?: string, signal?: AbortSignal): Promise<FungibleTokenPage> {
	...
	for (const { node } of connection.edges) {
		...
		assets.set(node.id, {
			...
			...(tags.logo && isArweaveId(tags.logo) ? { image: arweaveDataUrl(tags.logo, contentGateway) } : {}),
		});
	}
...
export async function loadImageCollection(
	...
	try {
		...
		throwIfAborted(signal);
		if (indexSource === 'compiled-fallback' || value === publishedManifestId || !isArweaveId(publishedManifestId)) {
			throw cause;
		}
...
export function carrierManifestId(state: Pick<AssetState, 'value'>): string | undefined {
	if (typeof state.value === 'string' && isArweaveId(state.value)) return state.value;
	...
	return typeof target === 'string' && isArweaveId(target) ? target : undefined;
}
...
function imageCollection(
	...
	if (
		...
		manifest.assets.some((asset) => {
			if (typeof asset === 'string') return !isArweaveId(asset);
			return (
				!asset ||
				typeof asset !== 'object' ||
