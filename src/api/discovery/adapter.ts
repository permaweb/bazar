...
export function loadCompletedWalletCandidateScan(
	...
	if (!isArweaveId(address)) return undefined;
	try {
		...
		if (
			...
			stored.h.length !== 3 ||
			stored.h.some((head) => head !== null && !isArweaveId(head)) ||
...
function decodeStoredWalletCandidate(value: StoredWalletCandidate): AssetCandidate | undefined {
	...
	if (
		...
		(activity !== undefined &&
			activity !== null &&
			activity !== 0 &&
			(!Array.isArray(activity) || activity.some((id) => typeof id !== 'string' || !isArweaveId(id)))) ||
...
export async function discoverMarketActivity(options: MarketActivityOptions = {}): Promise<AssetCandidate[]> {
	...
	const graphql = options.graphql ?? arweaveGraphqlEndpoint();
	const found = new Map<string, AssetCandidate>();
	const recipients = [...new Set((options.recipients ?? []).filter((id) => isArweaveId(id)))];
...
function atomicProcessNode(node: GraphqlNode): boolean {
	...
	return (
		...
		(!tags['asset-data'] || isArweaveId(tags['asset-data'])) &&
		(!tags['asset-artwork'] || isArweaveId(tags['asset-artwork'])) &&
...
function candidateFromNode(
	...
	if (!processId || !isArweaveId(processId)) return null;
...
function activityEventFromNode(node: GraphqlNode): CollectionActivityEvent | null {
	...
	return {
		...
		actor: isArweaveId(node.owner?.address ?? '') ? node.owner!.address! : '',
		height: safeNumber(node.block?.height),
		timestamp: safeNumber(node.block?.timestamp),
		...
		/^[1-9]\d*$/.test(
			...
			: {}),
		...(['register-interest', 'cancel-order'].includes(action) && isArweaveId(tags['order-id'] ?? '')
			? { orderId: tags['order-id'] }
			: {}),
		...(action === 'transfer' && isArweaveId(tags.recipient ?? '') ? { recipient: tags.recipient } : {}),
	};
}
