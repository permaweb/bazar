...
describe('collection index loading', () => {
  ...
  it('settles in the same canonical order regardless of source completion timing', () => {
    ...
    const finish = (snapshots: Collection[][]) => {
      const progress = snapshots.slice(0, -1).reduce((current, next) => mergeCollectionSnapshots(current, next), []);
      return mergeCollectionSnapshots(progress, snapshots.at(-1) ?? [], true);
    };

    expect(
      finish([[canonical[3]], [canonical[1], canonical[3]], [canonical[0], canonical[1], canonical[3]], canonical]).map(
        ({ id }) => id,
      ),
    ).toEqual(canonical.map(({ id }) => id));
    expect(
      finish([[canonical[0]], [canonical[0], canonical[2]], [canonical[0], canonical[2], canonical[3]], canonical]).map(
        ({ id }) => id,
      ),
    ).toEqual(canonical.map(({ id }) => id));
  });
  ...
  it('discovers every fungible token across GraphQL pages without duplicate assets', async () => {
    ...
    expect(
      fetcher.mock.calls.filter(([, init]) => typeof init?.body === 'string' && init.body.includes('FungibleTokens')),
    ).toHaveLength(2);
  });
