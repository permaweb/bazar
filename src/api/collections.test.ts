...
const encodeJson = (value: unknown) => btoa(JSON.stringify(value))
	.replaceAll('+', '-')
	.replaceAll('/', '_')
	.replaceAll('=', '');
...
describe('collection index loading', () => {
	...
	it('recognizes an unloaded fungible token only from its exact live process contract', () => {
		...
		expect(collectionAsset(tokens, processId, {
			...state,
		...
		vi.stubGlobal('fetch', vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
			...
			return Response.json({ data: { transactions: {
				...
				edges: Array.from({ length: 50 }, (_, index) => ({
					...
					node: { id: asset('N', 100 + index).id, tags: [{ name: 'Name', value: `N ${100 + index}` }] },
				})),
			} } });
		}));
	...
	it('preserves verified positions during progress and can reconcile canonical final order', () => {
		...
		expect(mergeCollectionSnapshots(
	...
	it('settles in the same canonical order regardless of source completion timing', () => {
		...
		const finish = (snapshots: Collection[][]) => {
			const progress = snapshots.slice(0, -1).reduce(
				(current, next) => mergeCollectionSnapshots(current, next),
				[],
			);
		...
		expect(finish([
			...
			canonical,
		]).map(({ id }) => id)).toEqual(canonical.map(({ id }) => id));
		expect(finish([
			[canonical[0]],
	...
	it('does not replace a paginated names snapshot with a stale first page', () => {
		...
		const current: Collection = {
			...
			kind: 'names',
			assets: [{ id: first, name: 'alice' }, { id: second, name: 'bob' }],
	...
	it('discovers every fungible token across GraphQL pages without duplicate assets', async () => {
		const tokenIds = [FUNGIBLE_TOKEN_ID, ...Array.from({ length: 100 }, (_, index) =>
			`${index.toString(36).padStart(42, '0')}A`
		)];
		...
		expect(fetcher.mock.calls.filter(([, init]) =>
			typeof init?.body === 'string' && init.body.includes('FungibleTokens')
		)).toHaveLength(2);
	});
	...
	it.each([
		['wrong manifest', { manifest: 'arweave/transactions', version: '0.2.0', paths: { alice: { id: 'A'.repeat(43) } } }],
