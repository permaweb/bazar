...
describe('wallet candidate discovery', () => {
  ...
  it('resumes a 16,000-candidate scan from its final failed page without replaying 15,900 candidates', async () => {
    ...
    const fetcher = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return Response.json({
        data: {
          initiallyHeld,
          ...(body.variables.includeMarket ? { marketActions: { pageInfo: { hasNextPage: false }, edges: [] } } : {}),
    ...
    const fetcher = vi.fn(async () =>
      Response.json({
        data: {
          transactions: {
            pageInfo: { hasNextPage: false },
            edges: [activityEdge('current-name', nameAsset, 20), activityEdge('outside-name', outsideName, 19)],
          },
        },
      }),
    );
  ...
  it('queries names activity globally, filters it locally, and verifies carrier processes', async () => {
    const pages = [
      {
        data: { transactions: { pageInfo: { hasNextPage: true }, edges: [activityEdge('outside', outsideName, 50)] } },
      },
      {
        data: {
          transactions: {
            pageInfo: { hasNextPage: false },
            edges: [activityEdge('carrier', nameAsset, 40), activityEdge('traditional', traditionalName, 30)],
          },
        },
      },
    ];
